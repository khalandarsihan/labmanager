    ...
    return (
      <div className="grid grid-cols-7 mb-2">
        {days.map(day => (
          <div
            key={day}
            className="h-12 flex items-center justify-center text-sm font-medium text-gray-400"
          >
            {day}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 gap-1">
        {calendarDays.map((date, index) => {
          ...
          return (
            <div
              key={index}
              className={`min-h-[100px] p-2 border rounded-md hover:border-blue-300 transition-colors duration-200 cursor-pointer ${
                isCurrentMonthDay 
            ...
            >
              <div className="flex justify-between items-center mb-1">
                <span className={`text-sm font-medium ${
                  isTodayDate ? 'bg-amber-400 text-white rounded-full w-6 h-6 flex items-center justify-center' : 'text-gray-700'
                }`}>
                  {date.getDate()}
                </span>
                {dayEvents.length > 0 && (
              ...
              <div className="space-y-1 overflow-hidden">
                {dayEvents.slice(0, 3).map(event => (
                  <div 
                    key={event.id} 
                    className={`text-xs p-1 rounded truncate ${event.color} text-white cursor-pointer hover:opacity-90 active:opacity-75 shadow-sm`}
                    onClick={(e) => {
                      e.stopPropagation();
        ...
        {sortedEvents.map(event => (
          ...
          >
            <div className="flex items-center">
              <div className={`${event.color} w-2 self-stretch`}></div>
                ...
                <div className="flex gap-6 mt-2 text-xs text-gray-500">
                  <div className="flex items-center gap-1">
                    ...
                    {new Date(event.start).toLocaleDateString() === new Date(event.end).toLocaleDateString()
                      ? formatDate(new Date(event.start), 'medium')
                      : `${formatDate(new Date(event.start), 'medium')} - ${formatDate(new Date(event.end), 'medium')}`
                    }
                  </div>
          ...
          {weekDays.map((date, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`mx-auto mt-1 w-8 h-8 rounded-full flex items-center justify-center cursor-pointer ${
                  isTodayDate 
              ...
              <div key={index} className="mb-4">
                ...
                <div className="ml-4 space-y-2">
                  {dayEvents.map(event => (
                    <div 
                      key={event.id}
                      className="bg-white border border-gray-200 rounded p-2 flex items-center gap-2 hover:shadow-md transition-shadow cursor-pointer"
                      onClick={() => setSelectedEvent(event)}
                    >
                      <div className={`${event.color} w-1 self-stretch rounded-full`}></div>
                      <div>
    ...
    return (
      <div className="space-y-3 max-h-[300px] overflow-y-auto">
        {selectedDateEvents.map(event => (
          ...
          >
            <div className="flex">
              <div className={`${event.color} w-1 self-stretch`}></div>
    ...
    <div className="relative">
      ...
      <div className="relative z-10 bg-gray-800/80 backdrop-blur-sm border border-gray-600 rounded-lg p-6 shadow-xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center border border-gray-700 rounded-md overflow-hidden">
              ...
              <button
                className="px-3 py-1 text-sm bg-gray-700 text-gray-200 hover:bg-gray-600 border-l border-r border-gray-600"
                onClick={goToToday}
              >
                Today
              </button>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6">
          <h3 className="text-xl font-medium text-amber-100 mb-2 md:mb-0">
            {view === 'week'
    ...
    <div className="bg-white/95 rounded-lg px-4 py-2 shadow-sm flex flex-wrap gap-3">
        ...
        {selectedEvent && (
          ...
          >
            ...
            >
              <div className={`w-full h-1 ${selectedEvent.color} rounded-full mb-4`}></div>
              
              <h3 className="text-xl font-semibold text-amber-300 mb-2">{selectedEvent.title}</h3>
