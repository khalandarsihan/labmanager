...
const BackgroundPattern = () => {
  ...
  return (
    <>
      {/* Full page background gradient - lowest z-index */}
      <div className={`fixed inset-0 -z-50 ${bgGradient} pointer-events-none`}></div>
      
      {/* Abstract Lines - next level z-index */}
      <div className={`fixed inset-0 -z-40 ${linesOpacity} pointer-events-none`}>
      ...
      {/* Floating Elements - highest z-index among background elements, but still below content */}
      <div className="fixed inset-0 -z-30 overflow-visible pointer-events-none">
        {/* Top Group */}
        <div className={`absolute top-20 left-1/3 w-14 h-14 border-2 ${borderColor}${elementsOpacity} rounded-lg backdrop-blur-sm transform rotate-45 animate-float-diagonal delay-300`} />
        <div className={`absolute top-24 right-1/3 w-10 h-10 border-2 ${borderColor}${elementsOpacity} transform -rotate-12 backdrop-blur-sm animate-float-up delay-700`} />
        
        {/* Left Side Elements */}
        <div className={`absolute top-1/2 left-20 w-16 h-16 border-2 ${borderColor}${elementsOpacity} rounded-full backdrop-blur-sm animate-float-circle delay-1000`} />
        <div className={`absolute bottom-1/3 left-1/4 w-12 h-12 border-2 ${borderColor}${elementsOpacity} transform rotate-12 backdrop-blur-sm animate-float-diagonal-reverse delay-500`} />
        
        {/* Right Side Elements */}
        <div className={`absolute top-1/2 right-20 w-12 h-12 border-2 ${borderColor}${elementsOpacity} rounded-lg transform rotate-45 backdrop-blur-sm animate-float-up-slow delay-200`} />
        <div className={`absolute bottom-1/3 right-1/4 w-14 h-14 border-2 ${borderColor}${elementsOpacity} rounded-full backdrop-blur-sm animate-float-side delay-900`} />
        
        {/* Additional Elements */}
        <div className={`absolute top-2/3 right-24 w-10 h-10 border-2 ${borderColor}${elementsOpacity} transform rotate-30 backdrop-blur-sm animate-float-diagonal delay-600`} />
        <div className={`absolute bottom-1/4 right-20 w-8 h-8 border-2 ${borderColor}${elementsOpacity} rounded-lg backdrop-blur-sm animate-float-circle delay-800`} />
      </div>
    </>
  );
};
