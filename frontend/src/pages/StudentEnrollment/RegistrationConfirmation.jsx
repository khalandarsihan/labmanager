...
const RegistrationConfirmation = ({ registrationId, studentData = {} }) => {
  ...
  useEffect(() => {
    if (registrationId && localStudentData && Object.keys(localStudentData).length > 0) {
      ...
      const timer = setTimeout(() => {
        handleDownloadConfirmation(false); // Direct download when page loads
      }, 1000);
  ...
  const buttonOutline = useLightTheme
    ...
    const handleDownloadConfirmation = async (openInNewTab = false) => {
      try {
        ...
        // Adjust vertical positions to account for logo and A3 size
        // const contentStartY = page.getHeight() - margin - logoHeight - 60;
